...
class CreateItem extends Component {
  ...
  uploadFile = async e => {
    ...
    data.append("upload_preset", "reactecom");
    const res = await fetch(cloudinaryUpload, { method: "POST", body: data });
        ...
        {(createItem, { loading, error }) => (
          ...
          >
            ...
            <fieldset disabled={loading} aria-busy={loading}>
              <label htmlFor="file">
                Image
                <input type="file" id="file" name="title" placeholder="Upload an image" required onChange={this.uploadFile} />
                {this.state.image && <img src={this.state.image} width="100" alt="Upload preview" />}
              </label>
              <label htmlFor="title">
                Title
